...
def summarize_performance(equity_curve: pd.Series, daily_pnl: pd.Series, ann_factor: int = 252) -> pd.Series:
    r = daily_pnl.dropna()
    if len(r) < 10:
    ...
    avg_turnover = float(r.index.to_series().map(lambda d: 1).mean())  # placeholder if needed
